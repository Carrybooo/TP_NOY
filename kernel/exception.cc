  ...
  case SYSCALL_EXCEPTION: {
	...
	case SC_PERROR: { 
	  ...
	  break;
        ...
        case SC_SEM_CREATE: {
            DEBUG('e', (char*)"Sync: Semaphore Create call.\n");
            int addr = g_machine->ReadIntRegister(4);
            int size = GetLengthParam(addr);
            char debug_name[size];
            GetStringParam(addr,debug_name,size);
			int count = g_machine->ReadIntRegister(5);
			Semaphore * sema = new Semaphore(debug_name, count);
			SemId sid = (SemId) g_object_ids->AddObject((void *) sema);
			g_syscall_error->SetMsg((char*)"", NO_ERROR);
		...
		case SC_SEM_DESTROY: {
			DEBUG('e', (char*)"Sync: Semaphore Destroy call.\n");
			SemId sid = (int) g_machine->ReadIntRegister(4);
			Semaphore * sema = (Semaphore *)g_object_ids->SearchObject(sid);
		...
		case SC_P: {
			DEBUG('e', (char*)"Sync: Semaphore P call.\n");
			SemId sid = (int) g_machine->ReadIntRegister(4);
			Semaphore * sema = (Semaphore *)g_object_ids->SearchObject(sid);
		...
		case SC_V: {
			DEBUG('e', (char*)"Sync: Semaphore V call.\n");
			SemId sid = (int) g_machine->ReadIntRegister(4);
			Semaphore * sema = (Semaphore *)g_object_ids->SearchObject(sid);
		...
		case SC_LOCK_CREATE: {
			DEBUG('e', (char*)"Sync: Lock Create call.\n");
			int addr = g_machine->ReadIntRegister(4);
			int size = GetLengthParam(addr);
			char debug_name[size];
			GetStringParam(addr,debug_name,size);
			...
			LockId lid = (LockId) g_object_ids->AddObject((void *) lock);
			g_syscall_error->SetMsg((char*)"", NO_ERROR);
		...
		case SC_LOCK_DESTROY: {
			DEBUG('e', (char*)"Sync: Lock Destroy call.\n");
			LockId lid = (int) g_machine->ReadIntRegister(4);
			Lock * lock = (Lock *)g_object_ids->SearchObject(lid);
		...
		case SC_LOCK_ACQUIRE: {
			DEBUG('e', (char*)"Sync: Lock Acquire call.\n");
			LockId lid = (int) g_machine->ReadIntRegister(4);
			Lock * lock = (Lock *)g_object_ids->SearchObject(lid);
		...
		case SC_LOCK_RELEASE: {
			DEBUG('e', (char*)"Sync: Lock Release call.\n");
			LockId lid = (int) g_machine->ReadIntRegister(4);
			Lock * lock = (Lock *)g_object_ids->SearchObject(lid);
		...
		case SC_COND_CREATE: {
            DEBUG('e', (char*)"Sync: Cond Create call.\n");
            int addr = g_machine->ReadIntRegister(4);
            int size = GetLengthParam(addr);
            char debug_name[size];
            GetStringParam(addr,debug_name,size);

            Condition * condition = new Condition(debug_name);
            CondId cid = (CondId) g_object_ids->AddObject((void *) condition);
            g_syscall_error->SetMsg((char*)"",NO_ERROR);
        ...
        case SC_COND_DESTROY: {
            DEBUG('e', (char*)"Sync: Cond Destroy call.\n");
            CondId cid = (int) g_machine->ReadIntRegister(4);
            Condition * Cond = (Condition *)g_object_ids->SearchObject(cid);
		...
		case SC_COND_WAIT: {
            DEBUG('e', (char*)"Sync: Cond Wait call.\n");
            CondId cid = (int) g_machine->ReadIntRegister(4);
            Condition * Cond = (Condition *)g_object_ids->SearchObject(cid);
		...
		case SC_COND_SIGNAL: {
            DEBUG('e', (char*)"Sync: Cond Signal call.\n");
            CondId cid = (int) g_machine->ReadIntRegister(4);
            Condition * Cond = (Condition *)g_object_ids->SearchObject(cid);
		...
		case SC_COND_BROADCAST: {
            DEBUG('e', (char*)"Sync: Cond Broadcast call.\n");
            CondId cid = (int) g_machine->ReadIntRegister(4);
            Condition * Cond = (Condition *)g_object_ids->SearchObject(cid);
